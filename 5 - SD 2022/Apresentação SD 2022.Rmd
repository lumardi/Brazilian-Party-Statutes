...
* Alteração do estatuto é só 1 caso de um continuum (não rígido) de mudanças institucionais:
  - Alterar estatuto -> Incorporar partido -> Fundir com partido -> Dissolver partido.
...
* Teorema de Imersão de Nash: para cada variedade de Riemann há uma imersão em um espaço preservada isometricamente. 

* Representação topológica fuzzy. 2 passos: aproximar o manifold dos dados, construir simplexos fuzzy que indexam a aplicação crescente.

* Distância entre clusters é melhor preservada, e não só intra clusters. 

* Cuidado: técnica de previsão, não de inferência. 

* Pressupostos: i) Manifold é diferenciável e conectado localmente; ii) Métrica Riemanniana é (aproximadamente) localmente constante; iii) Termos estocásticos são i.i.d. localmente.
